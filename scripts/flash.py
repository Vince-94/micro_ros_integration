...
def run_cmd(cmd: List[str] | str, cwd: Optional[Path] = None, env: Optional[Dict[str, str]] = None) -> None:
    ...
    try:
        ...
        fatal(f"Command failed (rc={e.returncode}): {' '.join(args)}", code=2)
    except FileNotFoundError:
        fatal(f"Command not found: {args[0]}", code=3)
...
# ---------- manifest ----------
def load_manifest(platform: str) -> (Dict[str, Any], Path):
    manifest_path = Path("platforms") / platform / "platform.yaml"
    if not manifest_path.exists():
        fatal(f"Platform manifest not found: {manifest_path}", code=4)
...
def find_rpi_rp2_mount(label: str = "RPI-RP2") -> Optional[Path]:
    """Try many heuristics to find a mounted RPI-RP2 mass-storage device."""
...
def flash_uf2(uf2_path: Path, mount_label: str = "RPI-RP2", wait_seconds: int = 0) -> None:
    if not uf2_path or not uf2_path.exists():
        fatal(f"UF2 file not found: {uf2_path}", code=5)
    ...
    if mp:
        ...
        try:
            ...
            fatal(f"Permission denied copying to {mp}. Try mounting with proper permissions or run with sudo.", code=6)
        except Exception as e:
            fatal(f"Failed to copy UF2: {e}", code=7)
        ...
        while time.time() < deadline:
            ...
            if mp:
                ...
                try:
                    ...
                    fatal(f"Failed to copy UF2 after mount detection: {e}", code=8)
            time.sleep(0.5)
    ...
    click.echo("  2) It should appear as a removable volume named 'RPI-RP2' (or similar).")
    click.echo(f"  3) Copy the UF2 file manually: cp {uf2_path} /path/to/RPI-RP2/")
    fatal("UF2 flashing aborted: device not mounted.", code=9)
...
def flash_picotool(manifest_path: Path, picotool_cmd: Optional[str], elf_path: Optional[Path], uf2_path: Optional[Path]) -> None:
    ...
    if picotool_cmd:
        ...
        run_cmd(full, cwd=manifest_path.parent)
        return
    ...
    if not pic:
        fatal("picotool not found. Either install it on the host or provide 'picotool_cmd' in platform.yaml.", code=10)

    if elf_path and elf_path.exists():
        run_cmd([pic, "load", str(elf_path.resolve())], cwd=manifest_path.parent)
    elif uf2_path and uf2_path.exists():
        run_cmd([pic, "load", str(uf2_path.resolve())], cwd=manifest_path.parent)
    else:
        fatal("picotool method selected but no ELF/UF2 found. Provide elf/uf2 or set picotool_cmd in manifest.", code=11)

def flash_openocd(manifest_path: Path, openocd_cmd: Optional[str], elf_path: Optional[Path], uf2_path: Optional[Path]) -> None:
    if not openocd_cmd:
        fatal("openocd method requested but no 'openocd_cmd' provided in manifest.", code=12)
...
def main(platform: str, method: Optional[str], uf2_override: Optional[str], elf_override: Optional[str], wait_seconds: int):
    ...
    if method == "uf2_copy":
        ...
        flash_uf2(uf2_path, mount_label=flash.get("uf2_mount_label", "RPI-RP2"), wait_seconds=wait_seconds)

    elif method == "picotool":
        picotool_cmd = flash.get("picotool_cmd")
        flash_picotool(manifest_path, picotool_cmd, elf_path, uf2_path)

    elif method == "openocd":
        openocd_cmd = flash.get("openocd_cmd")
        flash_openocd(manifest_path, openocd_cmd, elf_path, uf2_path)

    else:
        fatal(f"Unsupported flash method: {method}", code=13)
