...
def stream_subprocess(cmd: List[str], cwd: Optional[Path] = None, env: Optional[Dict[str, str]] = None) -> None:
    click.echo("+ " + " ".join(cmd))
...
class CMakeBackend(BuildBackend):
    ...
    def configure(self) -> None:
        ...
        stream_subprocess(cmd, cwd=self.build_dir, env=self.env)
...
class PlatformIOBackend(BuildBackend):
    ...
    def build(self) -> None:
        ...
        cmd.extend(self.extra_args)
        # run
        stream_subprocess(cmd, cwd=self.entry, env=self.env)
